...
def eval_det_cls(pred, gt, iou_thr=None, metric=None):
    ...
    for img_id in tqdm(pred.keys()):
        ...
        if len(gt_cur) > 0:
            ...
            iou_cur = pred_cur.overlaps(pred_cur, gt_cur, metric=metric)
...
def eval_map_recall(pred, gt, ovthresh=None, metric=None):
...
def indoor_eval(gt_annos,
    ...
    Args:
        ...
        iou_threshs (list[float]): IoU thresholds for computing average precisions.
    ...
    gt = {}  # map {class_id: gt}
    for img_id in tqdm(range(len(dt_annos))):
    ...
    rec, prec, ap = eval_map_recall(pred, gt, iou_threshs, metric)
    ret_dict = dict()
    header = ['classes']
