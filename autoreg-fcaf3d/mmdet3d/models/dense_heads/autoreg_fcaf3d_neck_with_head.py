...
class AutoregFcaf3DNeckWithHead(nn.Module):
    def __init__(self,
                 n_classes,
                 in_channels,
                 out_channels,
        ...
        self.autoreg = nn.ModuleList(
            [
                MLP(self.hdim + i + 1, [self.hdim, self.hdim * 2, self.hdim * 4, self.QUANT_ORDER[i][2]])
    ...
    def _loss_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            auto_losses = []
            feats_scale = torch.cat([pos_bbox_preds, pred_scale], dim=1)
