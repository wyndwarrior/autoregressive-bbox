...
from mmdet3d.models.dense_heads.autoreg_fcaf3d_neck_with_head import MLP, dequantize, quantize

@HEADS.register_module()
class AutoregDeltaFcaf3DNeckWithHead(nn.Module):
    def __init__(self,
                 n_classes,
                 in_channels,
                 out_channels,
        ...
        self.autoreg = nn.ModuleList(
            [
                MLP(self.hdim + i + 7, [self.hdim, self.hdim * 2, self.hdim * 4, self.QUANT_ORDER[i][2]])
    ...
    def _loss_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            gt_offset = pos_bbox_targets[:, :3]
            gt_lengths = pos_bbox_targets[:, 3:6]
            gt_euler = pos_bbox_targets[:, 6:]
            ...
            pred_scale = pred_bbox[:, 3:6]
            scaled_gt_lengths = (gt_lengths - pred_bbox[:, 3:6])/pred_scale
            scaled_gt_offset = (gt_offset - pred_bbox[:, :3])/pred_scale

            perm_losses = defaultdict(list)
            for flip in [True, False]:
                perm_losses['overflow_count'].append(torch.zeros(1, device=centerness.device))
                if flip:
                    orig_all = torch.cat([
                        scaled_gt_lengths, scaled_gt_offset,
                        ((gt_euler + np.pi/2)% np.pi +np.pi) % np.pi ], dim=1)
                else:
                    orig_all = torch.cat([scaled_gt_lengths, scaled_gt_offset, gt_euler], dim=1)
                quant_all = []
                dequant_all = []
                for val, quant_params in zip(orig_all.permute(1, 0), self.QUANT_ORDER):
                    quant, overflow = quantize(val, *quant_params)
                    perm_losses['overflow_count'][-1] += overflow.sum().float()
                    # if overflow.any():
                ...
                iou = cal_iou_3d(
                    torch.cat([
                        dequant_all[:, 3:6] * pred_scale + pred_bbox[:, :3],
                        dequant_all[:, :3] * pred_scale + pred_bbox[:, 3:6],
                        dequant_all[:, 6:],
                    ], dim=1)[None], 
                pos_bbox_targets[None])

                perm_losses['quant_iou_min'].append(iou.min())
                perm_losses['quant_iou_mean'].append(iou.mean())
                # print(iou.min(), iou.mean(), quant_all.max(0).values)

                auto_losses = []
                feats_scale = torch.cat([pos_bbox_preds, 
                    (pred_bbox[:, :3]-pos_points)/pred_scale, 
                    (pred_bbox[:, 3:6]-pos_points)/pred_scale, 
                    (pred_bbox[:, 6:7] % np.pi + np.pi) % np.pi], dim=1)
                ...
                perm_losses["loss_autoreg"].append(sum(auto_losses) / len(auto_losses) * 0.1)

            return_dict.update({
                k:torch.stack(v).min(0).values if 'quant' not in k
                else torch.stack(v).max(0).values for k, v in perm_losses.items()
            })

            # with torch.no_grad():
            #     beam_lengths, beam_pos, beam_angles, logps = self.sample_bbox(pos_bbox_preds, 10, 'beam')
            #     beam_bboxes = torch.cat([beam_pos[:, 0] + pos_points, beam_lengths[:, 0], beam_angles[:, 0]], dim=1)
    ...
    def sample_bbox(
        ...
        # pred_scale = self.predict_scale(bbox3d_feats)
        ...
        # feats_scale = torch.cat([bbox3d_feats, pred_scale], dim=1)
        feats_scale = torch.cat([bbox3d_feats, 
            (pred_bbox[:, :3]-points)/pred_scale, 
            (pred_bbox[:, 3:6]-points)/pred_scale, 
            (pred_bbox[:, 6:7] % np.pi + np.pi) % np.pi], dim=1)
        ...
        lengths = generated[:, :, :3] * pred_scale[:, None] + pred_bbox[:, None, 3:6]
        pos = generated[:, :, 3:6] * pred_scale[:, None] + pred_bbox[:, None, :3]
        angles = pred_bbox[:, None, 6:]#generated[:, :, 6:]
        return lengths, pos, angles, logps
    ...
    def _get_bboxes_single(self,
        ...
        for centerness, bbox_pred, cls_score, point in zip(
            ...
            lengths, pos, angles, logps = self.sample_bbox(bbox_pred, point, 50, 'beam')
